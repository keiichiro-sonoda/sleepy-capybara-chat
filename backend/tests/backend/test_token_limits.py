...
from app.schemas.enums import AIModelId, PeriodUnit, MetricType
from app.schemas.chat import AIModel, AVAILABLE_MODELS
...
class TestTokenLimitRollingWindows:
    ...
    async def test_daily_rolling_window(self, db: Session, test_user: User) -> None:
        ...
        create_token_usage(db, test_user.id, effective_tokens=300, 
                          timestamp=now - timedelta(hours=12))
        create_token_usage(db, test_user.id, effective_tokens=400,
                          timestamp=now - timedelta(hours=6))
        
        create_token_usage(db, test_user.id, effective_tokens=500,
    ...
    async def test_hourly_rolling_window(self, db: Session, test_user: User) -> None:
        ...
        create_token_usage(db, test_user.id, effective_tokens=600,
                          timestamp=now - timedelta(minutes=30))
        
        create_token_usage(db, test_user.id, effective_tokens=400,
                          timestamp=now - timedelta(hours=2))
        
        create_token_limit(db, test_user.id, limit_value=500,
                          period_unit=PeriodUnit.HOUR, period_value=1)
    ...
    async def test_monthly_rolling_window(self, db: Session, test_user: User) -> None:
        ...
        create_token_usage(db, test_user.id, effective_tokens=5000,
                          timestamp=now - timedelta(days=15))
        create_token_usage(db, test_user.id, effective_tokens=3000,
                          timestamp=now - timedelta(days=25))
        
        create_token_usage(db, test_user.id, effective_tokens=2000,
                          timestamp=now - timedelta(days=35))
        
        create_token_limit(db, test_user.id, limit_value=10000,
                          period_unit=PeriodUnit.MONTH, period_value=1)
    ...
    async def test_minute_rolling_window(self, db: Session, test_user: User) -> None:
        ...
        create_token_usage(db, test_user.id, effective_tokens=150,
                          timestamp=now - timedelta(seconds=30))
        
        create_token_usage(db, test_user.id, effective_tokens=100,
                          timestamp=now - timedelta(minutes=2))
        
        create_token_limit(db, test_user.id, limit_value=200,
                          period_unit=PeriodUnit.MINUTE, period_value=1)
    ...
    async def test_multi_period_rolling_window(self, db: Session, test_user: User) -> None:
        ...
        create_token_usage(db, test_user.id, effective_tokens=800,
                          timestamp=now - timedelta(days=2))
        create_token_usage(db, test_user.id, effective_tokens=600,
        ...
        create_token_limit(db, test_user.id, limit_value=2000,
                          period_unit=PeriodUnit.DAY, period_value=7)
...
class TestTokenLimitBreaches:
    ...
    async def test_limit_breach_returns_false_with_message(self, db: Session, test_user: User) -> None:
        ...
        create_token_usage(db, test_user.id, effective_tokens=1200,
    ...
    async def test_exact_limit_boundary(self, db: Session, test_user: User) -> None:
        ...
        create_token_usage(db, test_user.id, effective_tokens=1000,
                          timestamp=now - timedelta(hours=1))
    ...
    async def test_cumulative_usage_exceeds_limit(self, db: Session, test_user: User) -> None:
        ...
        create_token_usage(db, test_user.id, effective_tokens=350,
                          timestamp=now - timedelta(hours=6))
...
class TestUserModelSeparation:
    ...
    async def test_per_user_limit_isolation(self, db: Session) -> None:
        ...
        create_token_usage(db, user1.id, effective_tokens=1200,
                          timestamp=now - timedelta(hours=1))
        create_token_limit(db, user1.id, limit_value=1000)
        
        create_token_usage(db, user2.id, effective_tokens=500,
                          timestamp=now - timedelta(hours=1))
    ...
    async def test_per_model_limit_isolation(self, db: Session, test_user: User) -> None:
        ...
        create_token_usage(db, test_user.id, model_id="qwen3:8b",
                          effective_tokens=1200, timestamp=now - timedelta(hours=1))
        
        create_token_usage(db, test_user.id, model_id="gpt-4.1-nano",
                          effective_tokens=500, timestamp=now - timedelta(hours=1))
        
        create_token_limit(db, test_user.id, model_id=AIModelId.QWEN3_8B, limit_value=1000)
        create_token_limit(db, test_user.id, model_id=AIModelId.GPT_4_1_NANO, limit_value=1000)
    ...
    async def test_cross_user_usage_isolation(self, db: Session) -> None:
        ...
        create_token_usage(db, user1.id, effective_tokens=2000,
                          timestamp=now - timedelta(hours=1))
...
class TestUsageStatsAggregation:
    ...
    async def test_get_usage_stats_by_model(self, db: Session, test_user: User) -> None:
        ...
        create_token_usage(db, test_user.id, model_id="qwen3:8b",
                          prompt_tokens=100, completion_tokens=50, effective_tokens=200,
                          timestamp=now - timedelta(days=5))
        create_token_usage(db, test_user.id, model_id="qwen3:8b",
                          prompt_tokens=150, completion_tokens=75, effective_tokens=300,
                          timestamp=now - timedelta(days=10))
        
        create_token_usage(db, test_user.id, model_id="gpt-4.1-nano",
                          prompt_tokens=200, completion_tokens=100, effective_tokens=600,
                          timestamp=now - timedelta(days=15))
        
        create_token_usage(db, test_user.id, model_id="qwen3:8b",
                          prompt_tokens=50, completion_tokens=25, effective_tokens=100,
                          timestamp=now - timedelta(days=35))
    ...
    async def test_get_usage_stats_different_time_periods(self, db: Session, test_user: User) -> None:
        ...
        create_token_usage(db, test_user.id, effective_tokens=100,
                          timestamp=now - timedelta(days=5))
        create_token_usage(db, test_user.id, effective_tokens=200,
                          timestamp=now - timedelta(days=15))
        create_token_usage(db, test_user.id, effective_tokens=300,
                          timestamp=now - timedelta(days=45))
        
        stats_7_days = await TokenUsageService.get_usage_stats_by_model(db, test_user.id, days=7)
        stats_30_days = await TokenUsageService.get_usage_stats_by_model(db, test_user.id, days=30)
